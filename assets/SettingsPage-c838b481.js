...
`,Ue=xe().shape({name:T().trim("Cannot include leading and trailing spaces").strict().min(2,"Too short name").max(20,"Too long name").required("Name should be filled"),birthDate:me().transform(function(t,a){return this.isType(t)?t:Re(a,"dd.MM.yyyy",new Date)}).min("1900-01-01","Date is too early").max(new Date,"Please enter a correct date").typeError("Please enter a valid date DD.MM.YYYY").required("Date of birth should be filled"),gender:T().oneOf(["male","female"]).required(),height:z().positive("Height should be positive").integer("Height should be integer").min(140,"Incorrect data for calculation").max(250,"Incorrect data for calculation").required("Height should be filled"),weight:z().positive("Weight should be positive").integer("Weight should be integer").min(40,"Incorrect data for calculation").max(200,"Incorrect data for calculation").required("Weight should be filled"),activityLevel:z().oneOf([1.2,1.375,1.55,1.725,1.9]).required(),goal:T().oneOf(["Lose Fat","Maintain","Gain muscle"]).required()}),Ye=i(ve)`
  max-width: 300px;
  display: flex;
  flex-direction: column;
  align-items: center;
